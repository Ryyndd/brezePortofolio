                    ...
                    <div class="shadow-md dark:bg-gray-800 sm:rounded-lg">
                        <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                            ...
                            <div class="w-full gap-4 md:w-1/2 flex justify-between">
                                <form class="flex items-center" action="{{ route('project.index') }}" method="GET" >
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td scope="row" class="flex items-center px-4 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                            <img class="w-10 h-10 rounded-full" src="{{ asset('/storage/project/'.$data->pro_image) }}" alt="{{ $data->pro_title }}">  
                        </td>
                        <td class="px-6 py-4">
                            {{ $data->pro_title }}
                        </td>
                        ...
                        <td class="px-6 py-4">
                            {{ $data->techstack()->get()->implode('tech_name' , ',') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ Str::limit($data->pro_description,50) }}
                        </td>
                        <td class="px-2 py-4 ">
                            {{-- <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('project.destroy', $data->pro_slug) }}" method="POST">
                                <a  href="{{ route('project.edit', $data->pro_slug) }}" type="button"  class="font-medium me-3 text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="font-medium text-red-600 dark:text-blue-500 hover:underline">Delete</button>
                            </form> --}}

                            <form id="delete-form-{{ $data->pro_slug }}" action="{{ route('project.destroy', $data->pro_slug) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a  href="{{ route('project.edit', $data->pro_slug) }}" type="button"  class="font-medium me-3 text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                            <button type="button" onclick="confirmDelete('{{ $data->pro_slug }}')" class="font-medium text-red-600 dark:text-blue-500 hover:underline">
                                Delete
                            </button>
                        </td>
                    </tr>
